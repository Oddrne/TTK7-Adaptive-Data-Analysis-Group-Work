...
def calculate_freq_DMN_amplitude(subject_count=140, mvmd_prefix="mvmd_decomposed_modes", frequency_idx=0, network_name="Default"):
...
def plot_sorted_amplitudes(
    ...
    results : list of dict
        Output from calculate_freq_DMN_amplitude().
    sort_key : str
...
def plot_low_high_similarity(
    ...
    title_right="LOW vs HIGH IMF amplitudes",
    figsize=(12, 5),
    point_size=35,
    alpha=0.85,
):
    """
    Make clarity plots for whether low-frequency DMN amplitudes vary similarly to high-frequency ones.

    Parameters
    ----------
    low_results, high_results : list of dict
        Outputs from calculate_freq_DMN_amplitude for low and high IMF.
        Each element must contain at least:
          - "subject"
          - low_key / high_key (default "dmn_mean_amplitude")
    low_key, high_key : str
        Field names to pull amplitudes from.
    """
    ...
    order = np.argsort(low_amp)  # ascending
    ...
    # Spearman = correlation of ranks (robust to nonlinearity)
    ...
    # (1) Rank-alignment / sorted comparison
    ax = axes[0]
    x = np.arange(len(subjects_sorted))
    ax.scatter(x, low_sorted, s=point_size, alpha=alpha, label="LOW IMF")
    ax.scatter(x, high_sorted, s=point_size, alpha=alpha, label="HIGH IMF")
    ax.plot(x, low_sorted, alpha=0.4)
    ax.plot(x, high_sorted, alpha=0.4)
    ax.set_title(title_left)
    ax.set_xlabel("Subjects (sorted by LOW IMF amplitude)")
    ax.set_ylabel("DMN mean amplitude")
    ax.grid(alpha=0.3)
    ax.legend()

    # (2) Low vs high scatter
    ax = axes[1]
    ax.scatter(low_amp, high_amp, s=point_size, alpha=alpha)
    ax.set_title(title_right)
    ax.set_xlabel("LOW IMF DMN mean amplitude")
    ax.set_ylabel("HIGH IMF DMN mean amplitude")
    ax.grid(alpha=0.3)
...
def plot_network_boxplot(network_names, network_amplitude_lists,
                         ...
                         title="Distribution of High-frequency Amplitudes Across Networks",
                         figsize=(10, 5)):
    """
...
#plot_imfs(np.load("mvmd_decomposed_modes0.npz")["modes"], roi_idx=0, tr=2.0)
DMN_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3)
Vis_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="Vis")
SalVentAttn_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="SalVentAttn")
SomMot_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="SomMot")
DorsAttn_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="DorsAttn")
Limbic_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="Limbic")
Cont_amplitudes_low = calculate_freq_DMN_amplitude(140, "mvmd_decomposed_modes", frequency_idx=3, network_name="Cont")
